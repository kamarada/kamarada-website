...
O **Raspberry Pi 4 modelo B** possui as seguintes especificações:

- _chip_ Broadcom BCM2711
    - quatro núcleos (_quad core_) Cortex-A72
    - frequência de 1,5GHz
    - arquitetura ARM v8 de 64 _bits_
...
Observe que esse é o preço apenas da placa do Raspberry Pi. Além dela, é necessário comprar: a fonte de alimentação, um cartão de memória micro-SD e um cabo micro-HDMI. Além disso, é opcional, mas recomendado, comprar uma capa (_case_) para proteger a placa.
...
Se você já tem um computador, então provavelmente tem _mouse_, teclado e monitor. É possível utilizá-los com o Raspberry Pi, que tem portas USB e HDMI. Senão, é necessário comprá-los também.
...
- leitor de cartão (para conectar o cartão de memória ao computador)
- dissipadores de calor (_heat sinks_)
- fonte de alimentação
- cabo de vídeo micro-HDMI
...
## Raspbian: Debian para Raspberry
...
Como estamos começando a mexer no Raspberry Pi, vamos instalar o sistema operacional usando o método recomendado pelo _site_ oficial do Raspberry Pi, que é o **NOOBS** – _New Out Of the Box Software_ ("novo _software_ de fábrica", em uma tradução livre, também parece um trocadilho com a gíria _noob_, que quer dizer "novato").

O NOOBS é o instalador que instala o Raspbian no cartão de memória. É a forma mais fácil de instalar o sistema operacional no Raspberry Pi. Além do Raspbian, o NOOBS também pode ser usado para instalar a distribuição [LibreELEC], focada em multimídia.
...
Leia a mensagem de alerta. Depois, clique em **Aplicar**:
...
O Raspberry Pi não tem um botão ou um interruptor de liga/desliga: conecte a fonte de alimentação a uma tomada na parede e depois ao Raspberry Pi e ele ligará.
...
{% include image.html src="/files/2019/09/noobs-setup-01-pt.jpg" %}
...
Para desligar o Raspberry Pi, abra o menu de aplicativos e clique em **Shutdown** (desligar):
...
Como eu disse, eu comprei um _kit_ que inclui dissipadores de calor, ventoinha e capa de acrílico. Eu preferi instalar o sistema e testar a placa (solta na mesa mesmo) antes de completar a montagem porque pensei que o teste era mais prioritário. No AliExpress, o comprador tem até 15 dias para abrir uma disputa com o vendedor após o recebimento do produto. Se algo estivesse errado, eu já reclamaria. Felizmente, tudo funcionou bem.

Quanto aos dissipadores de calor, não encontrei instrução de como instalá-los. Instalei cada dissipador no _chip_ que aparentava ter o mesmo tamanho. Assim, cobri com os dissipadores o processador, a memória RAM e o controlador USB:

{% include image.html src="/files/2019/09/raspberry-pi-4-heat-sinks.jpg" %}

Você pode confrontar essa foto com o [diagrama de componentes](#components) do início.
